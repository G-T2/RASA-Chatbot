...
class ValidateContactForm(FormValidationAction):
    ...
    async def validate_phone_number(
        ...
        print(f"Raw phone number input: {slot_value}")
    
        phone_number = ''.join(c for c in slot_value if c.isdigit())
        
        print(f"Sanitized phone number (only digits): {phone_number}")
    
        if len(phone_number) == 10:  # Assume valid North American 10-digit number
            formatted_phone = f"{phone_number[:3]}-{phone_number[3:6]}-{phone_number[6:]}"
        elif len(phone_number) == 11 and phone_number.startswith('1'):  # Handle leading '1' for country code
            formatted_phone = f"{phone_number[0]}-{phone_number[1:4]}-{phone_number[4:7]}-{phone_number[7:]}"
        else:
            dispatcher.utter_message(text="Please provide a valid phone number with 10-digits. Formats accepted: '588-888-8888' or '588 888 8888'.")
